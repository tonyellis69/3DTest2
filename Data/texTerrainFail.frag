...
void main() {  
	...
	if (showDetails) {
		...
		//what tile are we in?
		vec2 tile = floor(texCoord0 * gridSize) * tileSize;
		//what offset within tile?
		//vec2 offset = fract(texCoord0 * gridSize);
		vec2 offset = smoothstep(0,1,fract(texCoord0 * gridSize));
		
		float q11 = smoothTile(tile + vec2(-halfTile));
		float q12 = smoothTile(tile + vec2(-halfTile,tileSize+halfTile));
		float q21 = smoothTile(tile + vec2(tileSize+halfTile,-halfTile));
		float q22 = smoothTile(tile + vec2(tileSize+halfTile,tileSize+halfTile));
